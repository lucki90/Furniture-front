...
export class ComponentVariantListComponent extends VariantListBase<ComponentVariantAdminResponse> implements OnInit {
  ...
  openCreateDialog(): void {
    this.dialog.open(VariantDialogComponent, {
      data: { type: 'component', mode: 'create' } as VariantDialogData,
  ...
  openEditDialog(variant: ComponentVariantAdminResponse): void {
    this.dialog.open(VariantDialogComponent, {
      data: { type: 'component', mode: 'edit', variant } as VariantDialogData,
      width: DIALOG_WIDTH.STANDARD,
    }).afterClosed().pipe(takeUntilDestroyed(this.destroyRef)).subscribe(result => {
      if (result) {
